...
var _Render = (map: GMap) => {
    ...
    if (map.marker) {
        if (map.marker.title) marker_opts["title"] = map.marker.title;
        if (map.marker.icon) marker_opts["icon"] = map.marker.icon;
        if (map.marker.shadow) marker_opts["shadow"] = map.marker.shadow;
    }
