...
export var GetTop = (elt:HTMLElement): number => {
    var _y = 0;
...
export var GetParameterByName = (name:string) => {
...
export var ToQryStr = (params: {[key:string]:string}) => {
    var str = [];
    for (var p in params)
        if (params.hasOwnProperty(p)) {
            str.push(encodeURIComponent(p) + "=" + encodeURIComponent(<string>params[<any>p]));
        }
    return str.join("&");
}
